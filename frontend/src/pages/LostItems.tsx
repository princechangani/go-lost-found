...
const LostItems: React.FC<LostItemsProps> = ({categories}) => {
    const { refresh } = useRefresh();



    const [allLostItems, setAllLostItems] = useState<Items[]>([]);
        ...
        apiClient.get<ItemsResponse>(API_ENDPOINTS.GET_ALL_ITEMS)
            .then((res) => {
                const LostItems = res.data.data
                    .filter((item) => item.type.toString().toLowerCase() === 'lost')
                    .map((item) => ({
                        ...item,
                        date: item.date ? new Date(item.date).toLocaleDateString('en-CA') : '',
                    }));
                setAllLostItems(LostItems);
            })
            .catch((err) => console.error(err));
    }, [refresh]);
    useEffect(() => {
        let filtered = [...allLostItems];
    ...
    }, [allLostItems, activeFilters, searchQuery]);
    ...
    return (
        <div className="space-y-6">
                        ...
                        <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                            ...
                            <div className="p-6">
                                <div className="flex justify-between items-start">
                                    ...
                                    >
                                        {item.statusType === 'found' ? 'Found' : 'Pending'}
                                    </span>
                                </div>
                                ...
                                <div className="mt-4 space-y-2">
                                    ...
                                    <div className="flex items-center text-sm text-gray-500">
                                        ...
                                        <span className="ml-2">{ categories.find(cat => cat.id === item.categoryId)?.name || 'Unknown'}</span>
                                    </div>
                                </div>
                                <button
            ...
            {/* Move ContactDialog outside the map function */}
            {selectedItem && (
                <ContactDialog
                    item={selectedItem}
                    open={openDialog}
